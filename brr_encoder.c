...
static pcm_t *resample(pcm_t *samples, size_t samples_length, size_t out_length, char type)
{
	...
	printf("Resampling by effective ratio of %d/%d (= %f)...\n", samples_length, out_length, ratio);
...
int main(const int argc, char *const argv[])
{
	...
	{
		printf("target_samplerate = true\n");
		target_length = ((long long)samples_length * target_samplerate) /  hdr.sample_rate;
	}
	else
	{
		printf("target_samplerate = false. ratio=%f\n", ratio);
		target_length = (int)(samples_length/ratio);
		printf("look at this shit... %d divided by %f equals %d\n", samples_length, ratio, target_length);
	}
	...
	if(fix_loop_en)
	{
		...
		// ^^ Nope! ^^
		new_loopsize = loopsize; //((loopsize + 15)/16)*16;
		// Adjust resampling
		...
		printf(
			...
			"...loopsize=%d\n"
			"...new_loopsize=%d\n"
			, ratio, samples_length, target_samplerate, hdr.sample_rate, loop_start, target_length, loopsize, new_loopsize);
	}
